    ...
    <div class="space-y-10 animate-fade-in">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p class="mt-1 text-sm text-gray-500">Manage your product categories and hierarchy.</p>
        </div>
        <app-button variant="primary" (click)="openAddModal()" class="shadow-lg shadow-yellow-500/20 hover:shadow-yellow-500/30 transition-shadow">
          <span class="mr-2 text-lg">+</span> Add Category
        </app-button>
      </div>
      ...
      <!-- Templates -->
      <ng-template #idTpl let-category>
      ...
      <ng-template #actionsTpl let-category>
         <button (click)="toggleActionMenu($event, category.id)" 
            ...
            <div class="sm:flex sm:items-start">
              ...
              <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                ...
                <div class="mt-2 text-sm text-gray-500">
                  <p class="mb-4">Fill in the details below to {{ editingCategory ? 'update the' : 'create a new' }} category.</p>
                  ...
                  <form (ngSubmit)="saveCategory()" class="mt-4">
        ...
        <div class="inline-block align-bottom bg-white rounded-2xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-md w-full border border-gray-100">
          <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
            <div class="sm:flex sm:items-start">
              ...
              <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                <h3 class="text-lg leading-6 font-bold text-gray-900">Delete Category</h3>
                <div class="mt-2">
                  <p class="text-sm text-gray-500">
                    Are you sure you want to delete <strong class="text-gray-900">{{ categoryToDelete?.name }}</strong>? This action cannot be undone.
                  </p>
          ...
          <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse border-t border-gray-100">
            <app-button variant="danger" [loading]="isDeleting" (click)="deleteCategory()" class="w-full sm:ml-3 sm:w-auto shadow-md">
              Delete
            </app-button>
...
export class CategoryListComponent implements OnInit {
  ...
  handleEdit() {
    if (this.activeActionMenuId) {
      const category = this.categories.find(c => c.id === this.activeActionMenuId);
      if (category) {
  ...
  saveCategory() {
    ...
    const request = this.editingCategory
      ? this.api.put(`/categories/${this.editingCategory.id}`, { name: this.categoryName.trim() })
      : this.api.post('/categories', { name: this.categoryName.trim() });
    ...
    request.subscribe({
      next: (res: any) => {
        ...
        if (res.success) {
          ...
          this.errorMessage = res.error || 'Failed to save category';
        }
      },
      error: (err) => {
        this.isSaving = false;
  ...
  deleteCategory() {
    ...
    this.api.delete(`/categories/${this.categoryToDelete.id}`).subscribe({
      next: (res: any) => {
        ...
        if (res.success) {
          ...
          this.deleteError = res.error || 'Failed to delete category';
        }
      },
      error: (err) => {
        this.isDeleting = false;
        this.deleteError = err.error?.error || 'Failed to delete category';
      }
    });
  }
}
